...
6. IMPORTANTE: Si hay correcciones de nave (vessel amendment/correction), incluir TANTO la nave original como la nave corregida. Campos: nave_original y nave_corregida. Las correcciones pueden estar tachadas, sobreescritas o anotadas sobre el mismo BL (texto tachado = valor original, texto nuevo = valor corregido). En algunos casos la corrección es un nombre de buque similar/cercano al original (no tachado), en ese caso si aparecen dos nombres de nave, el primero es el original y el segundo es el corregido.
7. IMPORTANTE: Si aparecen DOS puertos de descarga en el BL, el primero es el puerto de descarga final y el segundo es el puerto de transbordo (o viceversa según contexto). SIEMPRE distinguir entre puerto_desembarque (destino final) y puerto_transbordo. Si el puerto de destino fue corregido, incluir puerto_destino_original y puerto_destino_corregido. Si el puerto de transbordo fue corregido sobre el mismo BL, incluir puerto_transbordo_original y puerto_transbordo_corregido. Si el número de viaje fue corregido, incluir viaje_original y viaje_corregido
