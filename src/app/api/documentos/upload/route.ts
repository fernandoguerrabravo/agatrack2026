...
4. Incluye información del shipper, consignee, notify party con direcciones completas. IMPORTANTE BL MASTER vs HOUSE: Si el BL tiene dos números, SIEMPRE identificar AMBOS: numero_bl_master (MBL) y numero_bl_house (HBL). Reglas para distinguirlos: el que tiene "(h)" o "HBL" o "House" es el HOUSE (hijo). El otro es el MASTER. El MBL es el número de la naviera real (para tracking). El HBL es del freight forwarder. SIEMPRE reportar ambos si existen. Usar numero_bl = MBL (Master). VALIDACIÓN BL: los números de BL son ALFANUMÉRICOS (letras mayúsculas + dígitos). NUNCA confundir: 1 (uno) con l (ele minúscula), 0 (cero) con O (letra O), I (i mayúscula) con 1 (uno). Los BL NUNCA tienen letras minúsculas — siempre MAYÚSCULAS + números.
