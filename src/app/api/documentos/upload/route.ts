...
5. Incluye datos de flete, nave, viaje, puertos. IMPORTANTE FLETE: Leer con MÁXIMA PRECISIÓN los valores de flete en la sección "FREIGHT" o "OCEAN FREIGHT" del BL. Identificar CADA línea de la tabla de freight por separado (OCEAN FREIGHT, BAF, CAF, THC, ISPS, etc.) con su monto individual. Luego SUMAR todos los montos individuales de la columna PREPAID y verificar que coincida con el TOTAL PREPAID impreso. Si NO coincide, RE-LEER cada dígito con cuidado (confusiones comunes: 5↔3, 8↔0, 1↔7, 6↔8, 4↔1). Reportar: flete_detalle (array con cada concepto y monto), flete_total_prepaid (el total impreso), flete_total_collect, moneda, condicion_flete. Los montos suelen tener 2 decimales y usar coma como separador de miles. NUNCA inventar montos.
  ...
  "texto_completo": "el texto completo del documento tal como fue extraído"
    ...
    if (isImage) {
      ...
      try {
        ...
        execSync(`gs -dNOPAUSE -dBATCH -sDEVICE=png16m -r600 -dTextAlphaBits=4 -dGraphicsAlphaBits=4 -sOutputFile="${tmpPng}-%03d.png" "${tmpPdf}"`, { timeout: 60000 });
