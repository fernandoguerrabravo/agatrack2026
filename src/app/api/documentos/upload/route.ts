...
3. Para BL: identifica CADA contenedor por separado con su número, sello, contenido detallado (pallets, bolsas, peso por contenedor, volumen, descripción de mercancía, HS code). IMPORTANTE: lee con MÁXIMA PRECISIÓN cada número de contenedor y cada dígito (no confundir 6 con 8, 0 con O, 1 con I, 5 con S). VALIDACIÓN: los números de contenedor SIEMPRE tienen EXACTAMENTE 11 caracteres: 4 LETRAS + 7 DÍGITOS (ejemplo: MSCU1234567). Si el número que lees tiene menos de 7 dígitos, te falta un dígito — revisa de nuevo. Si tiene más de 4 letras, estás incluyendo algo extra. El último dígito es un CHECK DIGIT calculado según ISO 6346. Si hay una sección "per container" o "per cntr" al final del listado (común en BLs de MSC/Mediterranean Shipping Company), esos detalles (peso, volumen, pallets, descripción) aplican a CADA UNO de los contenedores listados arriba — replicar esos datos en cada contenedor del array. Indicar SIEMPRE el número de pallets por contenedor (campo "pallets") si aparece en el documento. Si dice "X PALLETS PER CONTAINER" o "X PLTS", ese es el número de pallets de cada contenedor.
