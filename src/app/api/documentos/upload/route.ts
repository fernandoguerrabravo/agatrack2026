...
3. Para BL: identifica CADA contenedor por separado con su número, sello, contenido detallado (pallets, bolsas, peso por contenedor, volumen, descripción de mercancía, HS code). IMPORTANTE: lee con MÁXIMA PRECISIÓN cada número de contenedor y cada dígito (no confundir 6 con 8, 0 con O, 1 con I, 5 con S). VALIDACIÓN: los números de contenedor tienen formato XXXX1234567 (4 letras + 7 dígitos) donde el último dígito es un CHECK DIGIT calculado según ISO 6346. Si un número no pasa la validación del check digit, revisa si confundiste algún dígito (especialmente 6↔8, 3↔8, 1↔7, 0↔6). Si hay una sección "per container" o "per cntr" al final del listado (común en BLs de MSC/Mediterranean Shipping Company), esos detalles (peso, volumen, pallets, descripción) aplican a CADA UNO de los contenedores listados arriba — replicar esos datos en cada contenedor del array. Indicar SIEMPRE el número de pallets por contenedor (campo "pallets") si aparece en el documento. Si dice "X PALLETS PER CONTAINER" o "X PLTS", ese es el número de pallets de cada contenedor.
4. Incluye información del shipper, consignee, notify party con direcciones completas
5. Incluye datos de flete, nave, viaje, puertos. IMPORTANTE FLETE: Leer con MÁXIMA PRECISIÓN los valores de flete en la sección "FREIGHT" o "OCEAN FREIGHT" del BL. NO CALCULAR ni sumar valores — leer EXACTAMENTE el número impreso en el documento. Identificar: monto del flete (valor numérico exacto tal como aparece impreso), moneda (USD, EUR, etc.), condición (PREPAID o COLLECT), y si hay cargos adicionales (BAF, CAF, THC, etc.) listarlos por separado con su valor exacto. El campo "PREPAID" indica el monto total prepagado y "COLLECT" el monto a cobrar en destino. Revisar AMBAS columnas PREPAID y COLLECT en la tabla de freight. NUNCA inventar o calcular montos — solo reportar lo que está escrito. VALIDACIÓN DE FLETE: si el total de flete no coincide con la suma de los componentes individuales, RE-LEER los dígitos con cuidado (confusiones comunes: 5↔3, 8↔0, 1↔7, 6↔8, 4↔1). Los montos de flete suelen tener 2 decimales y usar coma como separador de miles.
