...
4. Incluye información del shipper, consignee, notify party con direcciones completas. IMPORTANTE BL MASTER vs HOUSE: Si el BL tiene dos números (uno marcado como MBL/Master y otro como HBL/House o con "(h)"), identificar ambos: numero_bl_master (MBL - el principal para tracking) y numero_bl_house (HBL - el hijo). En BLs de freight forwarders como Danmar Lines, el número principal para tracking en navieras es el MBL. Reportar siempre numero_bl como el MBL (Master Bill of Lading). VALIDACIÓN BL: los números de BL son ALFANUMÉRICOS (letras mayúsculas + dígitos). NUNCA confundir: 1 (uno) con l (ele minúscula), 0 (cero) con O (letra O), I (i mayúscula) con 1 (uno). Los BL NUNCA tienen letras minúsculas — siempre MAYÚSCULAS + números.
5. Incluye datos de flete, nave, viaje, puertos. IMPORTANTE FLETE: Leer EXACTAMENTE cada valor impreso en la sección de freight del BL. El valor del flete es SOLO el TOTAL de la columna PREPAID. NO sumar PREPAID + COLLECT. NO incluir valores de la columna COLLECT en el total. Si hay dos columnas (PREPAID y COLLECT), reportar SOLO el valor de PREPAID como flete_total_prepaid. Reportar OBLIGATORIAMENTE: flete_detalle (array con cada línea: {concepto, monto, moneda}), flete_total_prepaid (SOLO columna PREPAID, NO sumar con COLLECT), moneda, condicion_flete ("PREPAID"). NUNCA omitir flete_total_prepaid. NUNCA sumar prepaid + collect.
