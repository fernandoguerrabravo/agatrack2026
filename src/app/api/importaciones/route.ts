...
// Operaciones de exportación a EXCLUIR
...
// Campos a excluir de la respuesta (mismos que exportaciones)
...
export async function GET(request: Request) {
  ...
  try {
    ...
    let sql = `SELECT * FROM out_despacho_fguerra WHERE operacion NOT IN (${placeholders}) AND rut_cliente = ?`;
    ...
    console.error("Importaciones query error:", message);
    return NextResponse.json(
      { error: "Error al consultar importaciones." },
      { status: 500 }
    );
  }
}
