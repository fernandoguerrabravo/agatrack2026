...
export async function confeccionarDIN(nroOperacion: string, docs: DocRow[]) {
  ...
  // Nave
  const naveRes = await resolverNave(nave);
  if (naveRes) { df.nav_id = naveRes.codigo; df.nav_nombre = naveRes.nombre; }
  ...
  for (const item of items) {
    ...
    // Consultar arancel
    const arancelHtml = await aduananetGet(`/modulos/din/dus_encabezado/consulta_arancel_json.php?partida=${dscPartida}&pais=${mf.pai_id_origen || "225"}&regimen=${regimen.regId}`);
    ...
    mf.mer_monto_ajuste_item = "0.00"; mf.mer_sig_ajuste = "+";
    ...
    console.log(`[confeccionar] Mercancía item POST: status=${mercRes.status} cod=${codigoProd} arancel=${dscPartida} cantidad=${cantidad} cif=${merCif}`);
  }
