...
export default function ImportacionesTable() {
  ...
  const fetchData = useCallback(async (desdeVal: string, hastaVal: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/importaciones?${params.toString()}`);
      if (!res.ok) {
  ...
  // Columnas dinámicas con url_factura y url_despacho al inicio
  ...
  function handleExport() {
    ...
    XLSX.writeFile(wb, "importaciones_export.xlsx");
  }

  // Render especial
  const PDF_COLUMNS = new Set(["url_factura", "url_despacho"]);
  const DATE_COLUMNS = new Set(["fecha_aceptacion", "autor_salida"]);
  const DOLLAR_COLUMNS = new Set(["total_fob", "valor_seguro", "valor_flete", "total_cif", "gravamenes_valor_1", "iva"]);
  ...
  function renderCell(col: string, value: unknown) {
    if (PDF_COLUMNS.has(col) && value && String(value).trim() !== "") {
      ...
      return (
        <a href={url} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-xs text-error gap-1" title={col === "url_factura" ? "Descargar Factura" : "Descargar Despacho"}>
          <PdfIcon /> PDF
        </a>
      );
    }
    if (DATE_COLUMNS.has(col)) return formatDate(value);
    if (DOLLAR_COLUMNS.has(col)) return formatDollar(value);
    return String(value ?? "");
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="card bg-base-100 shadow-sm">
        <div className="card-body p-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3">
            ...
            <button className="btn btn-primary btn-sm" onClick={handleBuscar} disabled={loading}>
              {loading ? <span className="loading loading-spinner loading-xs" /> : "Actualizar"}
            </button>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <input type="text" placeholder="Filtrar por cualquier campo..." className="input input-bordered w-full sm:max-w-sm" value={filter} onChange={(e) => setFilter(e.target.value)} />
        <div className="flex items-center gap-2">
          <span className="text-sm text-base-content/60">{filtered.length} registro{filtered.length !== 1 ? "s" : ""}</span>
        ...
        <>
          <div className="overflow-x-auto rounded-lg border border-base-300 bg-base-100">
            <table className="table table-sm table-zebra">
              ...
              <tbody>
                {paginated.length === 0 ? (
                  <tr><td colSpan={columns.length || 1} className="text-center py-8 text-base-content/50">No se encontraron resultados.</td></tr>
                ) : (
                  paginated.map((row, i) => (
                    <tr key={i}>
                      {columns.map((col) => (
                        <td key={col}
                          className={col === "referencias" ? "whitespace-normal break-words text-sm leading-tight" : "whitespace-nowrap max-w-xs truncate"}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="join">
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, idx) => {
                ...
                return (
                  <button key={pageNum} className={`join-item btn btn-sm ${page === pageNum ? "btn-active" : ""}`} onClick={() => setPage(pageNum)}>{pageNum}</button>
                );
              })}
...
function PdfIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 13h2m-2 3h4m4-9h-4a1 1 0 01-1-1V3" />
    </svg>
  );
}
